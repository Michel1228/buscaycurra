/**
 * /cursos/[tipo] — Ficha de un tipo de curso.
 *
 ...
 * ya le sirve. En la fase 2 se le añaden debajo las plazas concretas cuando las
 * haya, sin tocar esta estructura.
 */
...
/** Bloque de un dato suelto de la fila de arriba. */
function Dato({ icono, etiqueta, valor, destacado }: {
  ...
  return (
    <div className="rounded-xl p-3" style={{ background: "#1e212b", border: "1px solid #2d3142" }}>
      ...
      <p className="text-sm font-bold" style={{ color: destacado ? "#22c55e" : "#f1f5f9" }}>
        {valor}
      </p>
    </div>
  );
...
export default async function FichaCurso({ params }: { params: Promise<{ tipo: string }> }) {
  const { tipo } = await params;
  const t = tipoPorSlug(tipo, "ES");
  if (!t) notFound();
  ...
  // Schema.org Course. Sin `provider` a propósito: esta ficha describe el TIPO
  // de curso, no una convocatoria concreta de un centro. Inventarse un proveedor
