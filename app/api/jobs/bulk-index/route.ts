 ...
 *
 * Corre en lotes paralelos de 8 queries a la vez.
 * Úsalo en bucle para cubrir todas las combinaciones:
 *
 *   offset=0    → combinaciones 0-199
 *   offset=200  → combinaciones 200-399
 *   ...
 *
 * Body:
 *   { secret, source, offset?, limit? }
 *
 * source:
 ...
 *
 * Response:
 *   { ok, insertados, procesados, total, fuentes, totalEnBD, siguiente }
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (source === "arbeitnow") {
      ...
      const [arb, rem] = await Promise.all([
        indexarBulkArbeitnow(),
        indexarBulkRemotive(),
      ]);
      result = {
      ...
      // "jooble" — bulk por combinaciones con offset
    ...
    const { count: totalEnBD } = await supabase
      .from("ofertas")
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    totalEnBD: total,
    objetivo: 400000,
    progresoPct: Math.round(((total ?? 0) / 400000) * 100),
    totalCombinaciones: TOTAL_COMBINACIONES,
    mensaje: `${total?.toLocaleString("es-ES")} / 400.000 ofertas`,
  });
}
